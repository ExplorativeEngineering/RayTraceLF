            ...
            for nRay in range(len(camPix)):  # iterate over the 164 rays
                for midpt in range(len(midsX[nRay])):  # number of midpoints
                    ...
                    if 0 <= x < voxNrX_ and \
                            0 <= y < voxNrYZ_ and \
                            0 <= z < voxNrYZ_:
                    # only midpoints within object space...
                    #if x < voxNrX_ and y < voxNrYZ_ and z < voxNrYZ_:
                    #    voxelsWithRays.append([x,y,z])
                        if voxel[x][y][z] is None:
                            #voxel[x][y][z] =  [[nRay, nZ, nY, lengthsList[nRay][midpt]]]
                            voxel[x][y][z] =  [struct.pack('BbbB', nRay, nZ, nY, int(lengthsList[nRay][midpt]*40))]
                            number_of_rays = number_of_rays + 1
                        else:
                            #ray = [nRay, nZ, nY, lengthsList[nRay][midpt]]
                            #voxel[x][y][z].append(ray)
                            packedRay = struct.pack('BbbB', nRay, nZ, nY, int(lengthsList[nRay][midpt]*48))
                            voxel[x][y][z].append(packedRay)
                            number_of_rays = number_of_rays + 1

                    #voxel[midsX[nRay][midpt] - 1][midsOffY[nY][nRay][midpt] - 1][
                    #            midsOffZ[nZ][nRay][midpt] - 1].append(ray)
                    # creates ray[nRay, nZ, nY, len, alt, azim]
...
def saveLightFieldVoxelRaySpace(filename, voxel):
    # Save to file...
    Path("lfvox/").mkdir(parents=True, exist_ok=True)
        ...
        if rays is None:
            ...
            for ray in range(len(rays)):
                if rays[ray] is not None:
                    ...
                    # TODO
                    intensity = value * length/48 * 3000
...
def runProjection(voxNrX, voxNrYZ, ulenses, camPix, angleList, voxel, path):
    offsets = [0, 0, 0]
    projectArray(samples.sample_lineY(16), "Line Y", offsets, voxNrX, voxNrYZ, ulenses, camPix, angleList, voxel, path)
    projectArray(samples.sample_lineZ(16), "Line Z", offsets, voxNrX, voxNrYZ, ulenses, camPix, angleList, voxel, path)
    projectArray(samples.sample_lineX(16), "Line X", offsets, voxNrX, voxNrYZ, ulenses, camPix, angleList, voxel, path)
    ...
    # projectArray(samples.sample_block(64), "Block16", offsets, voxNrX, voxNrYZ, ulenses, camPix, angleList, voxel, path)
...
# TODO save images with different parameters to separate directories
# ======================================================================================
# Main...
# ======================================================================================
def main():
...
if __name__ == "__main__":
    ...
    gamma = 0.5  # for matplotlib (not tiff file) images:
    #============================================================================================
